...
def generate_random_logs(num_files, num_lines, num_string_per_line, num_strings):
    os.chdir("random/")
    strings = []
    count_string = []
    for i in range(num_strings):
        strings.append(randomString())
        count_string.append(0)
    
    for i in range(num_files):
        f = open(f'random{i}.log', 'w+')
        for j in range(num_lines):
            line = []
            for k in range(num_string_per_line):
                index = randint(0, num_strings) - 1
                count_string[index] += 1
                line.append(strings[index])
            strline = ' '.join(a for a in line) + '\n'
            f.write(strline)
...
generate_random_logs(num_files=10, num_lines=10000, num_string_per_line=10, num_strings=1000)
